...
def find_port(prefer_keyword="Moxa"):
    ...
    # Optionally prefer a port whose description matches a keyword (e.g. "Moxa")
    if prefer_keyword:
        for p in ports:
            if prefer_keyword.lower() in p.description.lower():
                print(f"Selected {p.device} ({p.description})")
                return p.device

    # Otherwise just take the first one in the list
...
def open_serial(port, baud, timeout=5, retries=3, delay=2):
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"Attempt {attempt} to open {port} failed: {e}")
            time.sleep(delay)
    raise serial.SerialException(f"Could not open {port} after {retries} attempts")

ser = open_serial(com_port, 38400, timeout=5)
time.sleep(1)
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Guarantee cleanup when the window X is clicked
...
if __name__ == "__main__":
    ...
    # Run the Tk event loop (handles WM_DELETE_WINDOW cleanly)
    app.mainloop()

    # Fallback cleanup in case mainloop exits without on_close firing
    if ser and ser.is_open:
        ser.close()